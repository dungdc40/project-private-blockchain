...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
          try {
            ...
            if (currentTime - time < 60 * 5) {
              ...
              reject(new Error("Can't add block because 5 minutes has expired"));
            }
          } catch (e) {
            reject(e.message);
          }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
          ...
          reject(new Error("Can't find block with specified hash"));
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
          for(let block of self.chain) {
            ...
            if(!result) {
              errorLog.push(`Block with hash ${block.hash} was tampered`);
            }

            if(block.height > 0 && block.previousBlockHash !== self.chain[block.height - 1].hash) {
              errorLog.push(`Block with hash ${block.hash} does not have correct previous hash`);
            }
          }
          resolve(errorLog);
        });
